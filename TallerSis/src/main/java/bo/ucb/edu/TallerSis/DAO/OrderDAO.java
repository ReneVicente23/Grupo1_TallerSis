...
public interface OrderDAO {
    ...
    @Select("SELECT p.id_order, p.total_payment, p.order_status_id_order_status, p.type_payment_id_typepay, p.delivery_id_delivery " +
            "FROM order p JOIN order_details j ON p.id_order = j.order_id_order JOIN dish q ON q.id_dish=j.dish_id_dish WHERE q.bussines_id_bussiness= #{idbussines};")
